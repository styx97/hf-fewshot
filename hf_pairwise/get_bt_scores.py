...
def parse_single_outcome(response, outcome_map, expand_options=default_expand_options):
    """
    Map a single response string to an outcome key.

    Returns the matching key from outcome_map (e.g. 1, 2, 3, 4) or None if no
    label matched.  Mirrors the per-result matching logic in get_pairwise_outcomes
    but operates on a single response and returns the key rather than appending
    to a list — useful for debiasing where we need to compare two responses for
    the same pair.
    """
    if not response:
        return None
    ...
    for key, val in outcome_map.items():
        if any(x in response for x in expand_options(val)):
            return key
    return None


def debias_pairwise_results(
    all_results: list,
    ...
    counterpart, as generated by create_pairwise_dataset(swap_pairs=True).

    Convention
    ----------
    Original pair  id: "{id1}_{id2}"  (single underscore)   text1=item1, text2=item2
    Swapped  pair  id: "{id2}__{id1}" (double underscore)   text1=item2, text2=item1

    Per-pair logic
    --------------
    outcome_map key 1 means "text1 wins", key 2 means "text2 wins".
    ...
    """
    original, swapped_map = {}, {}
    for r in all_results:
        if '__' in r['id']:
            id2, id1 = r['id'].split('__', 1)
            canonical = f"{id1}_{id2}"
            swapped_map[canonical] = r
        else:
            original[r['id']] = r
    ...
    for key, orig_r in original.items():
        ...
        orig_key = parse_single_outcome(orig_r['response'], outcome_map, expand_options)
        swap_key = parse_single_outcome(swap_r['response'], outcome_map, expand_options)

        if orig_key is None or swap_key is None:
            n_skip += 1
            continue
        ...
        agree = (orig_key == 1 and swap_key == 2) or (orig_key == 2 and swap_key == 1)
        if agree:
            debiased.append(orig_r)
            n_agree += 1
        else:
    ...
    for canonical, swap_r in swapped_map.items():
        if canonical not in original:
            orig_key = parse_single_outcome(swap_r['response'], outcome_map, expand_options)
            if orig_key == 1:
                flipped_response = outcome_map[2]
            elif orig_key == 2:
                flipped_response = outcome_map[1]
            else:
                flipped_response = swap_r['response']
            debiased.append({'id': canonical, 'response': flipped_response})
            n_single += 1
